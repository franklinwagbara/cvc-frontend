...
import { CoQData } from '../coq-application-pp-form.component';
import { AuthenticationService } from 'src/app/shared/services';

@Component({
  selector: 'app-coq-application-preview-pp',
  templateUrl: './coq-application-preview-pp.component.html',
  styleUrls: ['./coq-application-preview-pp.component.css'],
})
export class CoqApplicationPreviewPPComponent implements OnInit {
  liquidColumns = [
    'dip',
    'waterDIP',
    'tov',
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<CoqApplicationPreviewPPComponent>,
    ...
    if (!this.isGasProduct) {
      ...
      this.tankData = this.tankData.map((item) => {
        const correctedLiqVolB4 =
          item.before.observedLiquidVolume + item.before.shrinkageFactor;
        const correctedLiqVolAft =
          item.after.observedLiquidVolume + item.after.shrinkageFactor;
      ...
      const sumOfTotalWtVacB4 = this.tankData.reduce(
        (accum, curr) => accum + curr?.before?.totalWeightVac,
        0
      );
      const sumOfTotalWtVacAft = this.tankData.reduce(
        (accum, curr) => accum + curr?.after?.totalWeightVac,
        0
      );
      this.grandTotalWeightVac = sumOfTotalWtVacAft - sumOfTotalWtVacB4;

      const sumOfTotalWtAirB4 = this.tankData.reduce(
        (accum, curr) => accum + curr?.before?.totalWeightAir,
        0
      );
      const sumOfTotalWtAirAft = this.tankData.reduce(
        (accum, curr) => accum + curr?.after?.totalWeightAft,
        0
      );
